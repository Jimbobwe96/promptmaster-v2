...
export default function Home() {
  ...
  useEffect(() => {
    const checkActiveSessions = async () => {
      const validSessions = await sessionManager.validateSessions();
      setActiveSessions(validSessions);
      setShowReconnectionModal(validSessions.length > 0);
    };

    checkActiveSessions();
  }, []);

  // Helper function to find active sessions
      ...
      if (key?.startsWith('lobby:')) {
        try {
          ...
          if (data) {
            ...
            // Ensure the session has the required properties
            if (session.lobbyCode && session.username) {
  ...
  const validateSessions = async (sessions: SessionData[]) => {
    ...
    for (const session of sessions) {
      try {
        ...
        if (validation.canReconnect) {
          ...
          sessionStorage.removeItem(`lobby:${session.lobbyCode}`);
        }
      } catch (error) {
  ...
  const handleClearAllSessions = () => {
    activeSessions.forEach((session) => {
      sessionManager.clearSession(session.lobbyCode);
    });
    setActiveSessions([]);
  ...
  return (
    <main className="min-h-screen bg-[#FAFBFF] relative overflow-hidden">
      ...
      {isJoinModalOpen && (
        <JoinLobbyModal onClose={() => setIsJoinModalOpen(false)} />
      )}
    </main>
  );
}
